...
///this cheat function will replace our network layer
func generateMovies (_ count: Int) -> [Movie] {
    ...
    for _ in 0..<count {
        ...
        let movie = Movie(id: id, name: name, thumbnailURL: thumbnail, categories: categories, genre: .AllGenres, year: year, rating:
                            "PG-13", numberOfSeasons: numSeasons, episodes: allEpisodes, promotionHeadline: headline, currentEpisode: defEpiInfo, defaultEpisodeInfo: defEpiInfo, creators: creators, cast:
                            cast, moreLikeThisMovies: moreLikeThis, trailers: allTrailers, previewImageName: "")
...
func generateEpisodes(_ count: Int, forSeason season: Int) -> [Episode] {
    ...
    for num in 0..<count {
        ...
        let episode = Episode(id: id, name: name, season: season, episodeNumber: num + 1, thumbnailImageURLString: thumbnail,
                              description: desc, length: randLength, videoURL: exampleVideoURL)
...
func randomString(length: Int) -> String {
    let letters = "abcdefghijklmnoparstuvwxyz"
